...
from src.setup import device, resize_size, feat_dim
...
from .sliding_window_dataset_for_labelled_dataset import get_labelled_data_generator
...
class FineTuner:
    ...
    def initialize_model(self):
        """
        Initialize the DINOv2 model with adapter layers and MLP head.

        The pre-trained DINOv2 model is loaded and frozen. Adapter layers are added to
        each block's MLP. A new MLP head is added to the model to predict the presence
        of a PSD. The entire model is then put into evaluation mode.

        Returns
        -------
        augmented_model : nn.Sequential
            The modified DINOv2 model.
        """
...
def get_attention_maps(self, image):
    # advised to use in eval mode
    attn = []
    def hook_fn(module, input, output):
        attn.append(module.attn_weights.detach().cpu())

    # Attach hook
    last_block = self.model[0].blocks[-1]
    last_block.attn.register_forward_hook(hook_fn)

    # Preprocess and forward
    img = resize_image(image).unsqueeze(0)
    img_tensor = torch.from_numpy(img) if type(img) == np.ndarray else img
    _ = self.model(img_tensor)

    # Process attention
    attn_map = attn[-1]   # [batch, heads, tokens, tokens]
    cls_attn = attn_map[0, :, 0, 1:]  # [heads, num_patches]
    heat = cls_attn.mean(0)           # average over heads
    patch_w = patch_h = resize_size // self.patch_size
    heat = heat.reshape(patch_h, patch_w)       # patch grid
    heat = F.interpolate(
    ...
    # Show overlay
    plt.imshow(T.ToPILImage()(img_tensor[0]))
...
def hook_fn(module, input, output):
    attn_maps = []
    # module is the attention sub-module of a transformer block
    # output is the block output; but the raw attention weights may be stored in module
    # (depending on implementation, you might capture module.attn_map or output[1] if returned)
    attn_maps.append(module.attn_map)  
...
def run_fine_tuning(nb_iterations=2, nb_epochs_per_iteration=10, 
    ...
    test_set = list(get_labelled_data_generator( # TODO: we use human-made ground-truth for validation
        split='test',
        #nb_best_patches=nb_best_patches,
        resize_size=resize_size, 
        padding_size=padding_size, 
...
if __name__ == '__main__':
    ...
    loss_lists, test_lists, trained_model = run_fine_tuning(
        ...
        nb_epochs_per_iteration=30,  # Reduced for faster training
        nb_best_patches=20,
        padding_size=2,
