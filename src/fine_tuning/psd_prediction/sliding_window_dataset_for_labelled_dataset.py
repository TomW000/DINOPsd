...
from src.analysis_utils import resize_hdf_image, resize_tiff_image
...
from src.setup import embeddings_path, model, feat_dim, device, resize_size, model


if platform.system() == 'Linux':
    dataset_path = '/home/tomwelch/Cambridge/Datasets/original/train'
    labelled_dataset_path = '/home/tomwelch/Cambridge/Datasets/labelled_dataset'
else:
...
if saved_embeddings:
    try:
        image_list = torch.load(os.path.join(labelled_dataset_path, 'image_list_518.pt'), map_location='cpu')
        gt_list = torch.load(os.path.join(labelled_dataset_path, 'gt_list_518.pt'), map_location='cpu')
    ...
    _, image_list, gt_list = compute_embeddings()
    torch.save(image_list, os.path.join(labelled_dataset_path, 'image_list_518.pt'))
    torch.save(gt_list, os.path.join(labelled_dataset_path, 'gt_list_518.pt'))
...
def get_labelled_data_generator(split: str = 'test', 
    ...
    for image, gt in tqdm(list_iterator, desc=f'Loading {split} set'):
            resized_image = resize_tiff_image(image, resize_size=resize_size).squeeze()
            ...
            try:
                GT = resize_tiff_image(gt, resize_size=resize_size).squeeze()
                GT = torch.from_numpy(GT) if type(GT) == np.ndarray else GT
            
            except Exception as e:
                print(f"Error generating ground truth: {e}")
                continue

            result = (image, patches_list, GT)
