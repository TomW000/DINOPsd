...
from .dataset import cross_validation_datasets_generator, sliding_window_datasets_generator, nb_best_patches 
...
def filter_dataset(dataset_generator, test_accuracy_list, accuracy_threshold: float=80.0):
    """
    Filters datasets based on test accuracy, selecting those below a given threshold.

    This function iterates over datasets generated by `dataset_generator`, identifying datasets where the test 
    accuracy is below the specified `accuracy_threshold`. It accumulates inputs and targets for both training and 
    testing sets from these challenging datasets, specifically selecting samples with targets equal to 0. 

    Args:
        dataset_generator: An iterable that yields tuples of training and testing data loaders.
        test_accuracy_list (list): A list of test accuracies corresponding to datasets in `dataset_generator`.
        accuracy_threshold (float, optional): The accuracy threshold below which datasets are considered challenging. 
                                              Defaults to 80.0.

    Returns:
        list: A list of tuples where each tuple contains filtered inputs and targets from challenging datasets.
    """
...
def training_main():
    ...
    for train_set, test_set, _ in tqdm(dataset_generator, desc='Looping through datasets', leave=False):
    ...
    FILTERED_LABELLED_REST_SET = filter_dataset(dataset_generator=dataset_generator, 
    ...
    latest_test_accuracy_list = []  
    for train_set, test_set, _ in tqdm(filtered_dataset_generator, desc='Looping through filtered datasets', leave=False):
        detection_head = Psd_Pred_MLP_Head(device=device, feat_dim=feat_dim)
        detection_head.to(device)
        test_accuracy = detection_head_training(nb_epochs=1, 
                                                detection_head=detection_head,
