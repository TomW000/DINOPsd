...
class LargeEmbeddingsProcessor:
    ...
    def load_embeddings_efficiently(self, file_path):
...
def main():
    ...
    plt.scatter(umap_all[:, 0], umap_all[:, 1], alpha=0.6, s=0.5, c=labels)
    plt.title(f'UMAP Visualization - ALL {len(umap_all)} embeddings')
    plt.xlabel('UMAP 1')
    plt.ylabel('UMAP 2')
    plt.show()
