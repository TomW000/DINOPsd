...
from .setup import dataset_path, directory_path, resize_size
...
def main():
    ...
    for file in files:
        ...
        if file_extension == '.tif' or file_extension == '.tiff':
            img = tifffile.imread(os.path.join(dataset_path, file)) # type: ignore
            z_dim, y_dim, x_dim = img.shape
            middle_slice = img[z_dim // 2]
            image = middle_slice
        ...
        display_image(image, resize_size=resize_size)
