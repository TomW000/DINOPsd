...
def compute_accuracies(reference_embeddings, 
    ...
    if len(embeddings.shape) > 2:
        ...
        print(f"Reshaped embeddings from {original_shape} to {embeddings.shape}")
    
    if len(reference_embeddings.shape) > 2:
        original_ref_shape = reference_embeddings.shape
        reference_embeddings = reference_embeddings.reshape(-1, reference_embeddings.shape[-1])
        print(f"Reshaped reference embeddings from {original_ref_shape} to {reference_embeddings.shape}")
...
def main():
    ...
    try:
        mean_refs = compute_ref_embeddings(
            ...
            embs_path=os.path.join(embeddings_path, 'giant_mean_ref_518_Aug=False_k=10.pt')
        )
    ...
    plt.title(f'Classification Accuracy by Neurotransmitter Class ({model_size} DINOv2)')
